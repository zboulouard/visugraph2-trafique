...
//*********************
    ...
    */
     ...
     */
    public Point3D nouvellePosition(int maxw, int maxh)
    {
     ...
     */
    public Sommet hitTest( Point3D pt, boolean large )
    {
	for ( int i = 0; i < sommets.size(); i++ ) {
	    Sommet s = getSommet(i);
	    if ( s.getVisible()) {
		if (large) {
		    double dist = Math.sqrt(Math.pow(pt.getX() - s.getX(), 2.0)+
					    Math.pow(pt.getY() - s.getY(), 2.0)+
					    Math.pow(pt.getZ() - s.getZ(), 2.0));
		    if ( dist <= 10 )
			return s;
		} else {
		    if ( s.getX() == pt.getX() && s.getY() == pt.getY() && s.getZ() == pt.getZ() )
			return s;
		}
	    }
	}
