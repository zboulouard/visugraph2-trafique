    ...
    public void mouseClicked(MouseEvent e) {
    	...
    	try{
       		...
       		if (centrer==true){
       			...
       			gravx/=nb_som;
       			gravy/=nb_som;
	    		if (gravx>(ptDrag.x) | gravy>(ptDrag.y)|gravx<(ptDrag.x) | gravy<(ptDrag.y)) {
		               for ( int i = 0; i < gr.nombreSommets(); i++ ) {
		        			Sommet s = gr.getSommet(i);
		        			if (!s.nom.contains("Repere") && !s.nom.contains("virtuel") && s.getVisible()){
				        			if (gravx>(ptDrag.x))  s.x-=(gravx-(ptDrag.x));
				        			if (gravy>(ptDrag.y))  s.y-=(gravy-(ptDrag.y));
				        			if (gravx<(ptDrag.x))  s.x+=((ptDrag.x)-gravx);
				        			if (gravy<(ptDrag.y))  s.y+=((ptDrag.y)-gravy);
				        			}
	                }
		               repaint();
		               centrer=false;
